...
public class StringKey implements IndexKey, Comparable, Serializable {
    ...
    StringKey(Property title, Property tags, Property fileAttributes, Property hashCode, Property importTime,
            Property indexLoc, Property filepath, Property wordOccurences) {
        ...
        parameters.put("accessAverage", parameters.get("accessCount") / parameters.get("lastAccessTime").getTime()
                - parameters.get("importTime").getTime);
    ...
    public int duplicateCheck(StringKey i) {
        /*
         * final DateFormat sdf = new SimpleDateFormat("yyyy/MM/dd HH:mm:ss"); // final
         * DateTimeFormatter dtf = DateTimeFormatter.ofPattern("yyyy/MM/dd HH:mm:ss");
         * Date d = new Date(); parameters.get("lastAccessTime") = sdf.format(d);
         */
        // Should it update the last access time on duplicate check?
        this.update();
        if (parameters.get("title") != i.getTitle() && parameters.get("hashCode") != i.getHashCode()
                && parameters.get("UUID") != i.getUUID()) {
            return 0;
        } else if (parameters.get("title") == i.getTitle() && parameters.get("hashCode") != i.getHashCode()
                && parameters.get("UUID") != i.getUUID()) {
            return 1;
        } else if (parameters.get("title") != i.getTitle() && parameters.get("hashCode") == i.getHashCode()
                && parameters.get("UUID") != i.getUUID()) {
